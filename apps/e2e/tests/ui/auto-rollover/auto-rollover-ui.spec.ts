...
test.describe("Auto rollover (UI)", () => {
  ...
  async function openDialog(page: import("@playwright/test").Page) {
    ...
    await expect(page.getByRole("heading", { name: "Automatic rollover" })).toBeVisible();
  }

  async function saveDialog(page: import("@playwright/test").Page) {
    const saveResponsePromise = page.waitForResponse((r) => {
      const url = r.url();
      return r.request().method() === "PATCH" && url.includes("/ledger/participations/auto-rollover-settings");
    });
    await page.getByRole("button", { name: "Save" }).click();
    const saveResponse = await saveResponsePromise;
    if (!saveResponse.ok()) {
      const body = await saveResponse.text().catch(() => "");
      throw new Error(`Save settings failed: ${saveResponse.status()} ${saveResponse.statusText()}\n${body}`);
    }
    await expect(page.getByRole("heading", { name: "Automatic rollover" })).toBeHidden();
  }

  test("can enable automatic rollover from Cycles page and see badge update", async ({ page }) => {
    test.skip(
  ...
  test("can enable Full payout and see badge update + persistence", async ({ page }) => {
    const prisma = createPrisma();
    try {
      await ensureE2eAdmin(prisma);
      await resetTradecraftData(prisma);
  ...
  test("can disable automatic rollover and see badge Off + persistence", async ({ page }) => {
    const prisma = createPrisma();
    try {
      await ensureE2eAdmin(prisma);
      await resetTradecraftData(prisma);
