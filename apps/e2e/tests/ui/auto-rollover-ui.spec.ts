...
test.describe("Auto rollover (UI)", () => {
  test.beforeEach(() => {
    ...
    try {
      ...
      const session = cookies.find(
        (c) => c?.name === "session" && typeof c?.value === "string" && c.value.length > 0,
      );
  ...
  test("can enable automatic rollover from Cycles page and see badge update", async ({
    ...
    try {
      ...
      if (process.env.E2E_UI_DEBUG === "1") {
        // Lets you inspect the page state interactively in headed mode.
        await page.pause();
      }

      // If session-cookie auth failed (typically ENCRYPTION_KEY mismatch with the running API),
      // the app will render a sign-in CTA. Optionally allow manual login during the test.
      const signInButton = page.getByRole("button", { name: /Sign in with EVE/i });
      if (await signInButton.isVisible().catch(() => false)) {
        if (process.env.E2E_UI_MANUAL_LOGIN === "1") {
          // Headed mode recommended. Log in via the UI, then resume.
          // Note: cookie storage is per-test, so this doesn't persist between tests.
          // If we want persistence, we can add a storageState workflow later.
          await page.pause();
        } else {
          throw new Error(
  ...
  test("can disable automatic rollover and see badge Off + persistence", async ({
    ...
    try {
      ...
      // First enable it (via UI) so disabling is meaningful.
      await page.goto("/tradecraft/cycles", { waitUntil: "networkidle" });
      await openDialog(page);
      await page.getByLabel("Enable automatic rollover").check();
      await page.getByLabel("Initial investment only").click();
