...
export default function StrategyRunDetailPageClient({ runId }: { runId: string }) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Run: {data.strategy.name}</h1>
          <p className="text-sm text-muted-foreground">
            {String(data.startDate).slice(0, 10)} → {String(data.endDate).slice(0, 10)} •{" "}
            {data.status}
          </p>
        </div>
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 md:grid-cols-4 text-sm">
            <div>
              ...
              <div className="font-medium tabular-nums">{formatIsk(Number(last.cashIsk))}</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="font-medium tabular-nums">{formatIsk(Number(last.navIsk))}</div>
            </div>
          </CardContent>
        </Card>
      )}
              ...
              >
                <div className="min-w-0">
                  <div className="text-sm font-medium truncate">
                    {p.typeName ?? `typeId=${p.typeId}`} • dst {p.destinationStationId}
                  </div>
                ...
                <div className="text-sm tabular-nums">{formatIsk(Number(p.realizedProfitIsk))}</div>
              </div>
            ))}
          </div>
        </CardContent>
