...
async function getTypeByName(
  typeName: string,
): Promise<{ id: number; volume: number } | null> {
...
async function buildPlanResult(parsedPlan: ParsedPlan) {
  ...
  for (const pkg of parsedPlan.packages) {
    ...
    if (!stationId) {
      console.warn(
        `⚠️  Unknown station: ${pkg.destinationStation}, skipping package`,
      );
      continue;
    }
...
function getArg(name: string): string | undefined {
  ...
  return idx >= 0 && idx < process.argv.length - 1
    ? process.argv[idx + 1]
    : undefined;
}
...
async function main() {
  ...
  if (!filePath) {
    ...
    console.log('\nUsage:');
    console.log(
      '  npx ts-node scripts/generate-plan-json.ts --file PATH [--output PATH] [--memo MEMO]',
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(
      `   Total items: ${planResult.packages.reduce((sum, p) => sum + p.items.length, 0)}`,
    );
