...
/**
 * Seed a handful of Strategy Lab presets.
 *
 * Usage:
 *   pnpm -C apps/api exec tsx scripts/seed-strategy-lab.ts
 *
 * Notes:
...
const SHIPPING_COST_BY_STATION: Record<string, number> = {
  "60004588": 20_000_000,
  "60005686": 15_000_000,
  "60008494": 25_000_000,
  "60011866": 15_000_000,
...
const BASE_PARAMS = {
  ...
  // We backtest from daily aggregates; keep liquidity window explicit.
  liquidityOptions: {
    windowDays: 14,
  ...
  arbitrageOptions: {
    // Fees: leave undefined to use server defaults (AppConfig.arbitrage().fees),
    // but you can pin them here if you want.
  },
};
...
function makeStrategies() {
  return [
    ...
    },
    {
      ...
      description:
        "Treat ISK as the scarce resource (like a capital efficiency strategy): bias toward ROI rather than space/density.",
      params: {
        ...BASE_PARAMS,
        densityWeight: 0.25,
    ...
    },
    {
      ...
      description:
        "Force diversification between hubs (reduces concentration risk); uses targetWeighted allocation with mild bias.",
      params: {
        ...BASE_PARAMS,
        allocation: {
          mode: "targetWeighted" as const,
          spreadBias: 0.5,
          targets: {
            "60004588": 0.25,
            "60005686": 0.25,
