...
- [x] An admin API endpoint returns market table size, index size, total size, row count, oldest timestamp, and newest timestamp where available.
- [x] The report includes known high-growth market tables, especially raw NPC market snapshots and run metadata.
- [x] The report includes daily aggregate market tables without implying they should be pruned.
- [x] The admin UI presents the report in an on-screen table.
- [x] The report handles empty or missing timestamp ranges without failing.
- [x] Tests cover the returned report shape and at least one representative table-size result.
